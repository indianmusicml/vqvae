...
from jukebox_vqvae.encdec import Encoder, Decoder, assert_shape
from jukebox_vqvae.bottleneck import NoBottleneck, Bottleneck
...
from audio_utils import spectral_loss, multispectral_loss, spectral_convergence
...
class VQVAE(nn.Module):
    def __init__(self, input_shape, levels, downs_t, strides_t,
        ...
        self.bandwidth = dict(l2=1., l1=1., spec=1.)
    ...
    def forward(self, x, hps, loss_fn='l1'):
        ...
        def _loss_fn(loss_fn, x_target, x_pred, hps):
            if loss_fn == 'l1':
                return t.mean(t.abs(x_pred - x_target)) / self.bandwidth['l1']
            elif loss_fn == 'l2':
                return t.mean((x_pred - x_target) ** 2) / self.bandwidth['l2']
            elif loss_fn == 'linf':
                residual = ((x_pred - x_target) ** 2).reshape(x_target.shape[0], -1)
                values, _ = t.topk(residual, hps.linf_k, dim=1)
                return t.mean(values) / self.bandwidth['l2']
            elif loss_fn == 'lmix':
                loss = 0.0
                if hps.lmix_l1:
        ...
        def _spectral_loss(x_target, x_out):
            sl = spectral_loss(x_target, x_out) / self.bandwidth['spec']
            sl = t.mean(sl)
            return sl

        def _multispectral_loss(x_target, x_out):
            sl = multispectral_loss(x_target, x_out) / self.bandwidth['spec']
            sl = t.mean(sl)
            return sl
        ...
        for level in reversed(range(self.levels)):
            ...
            # this_spec_loss = _spectral_loss(x_target, x_out)
            this_multispec_loss = _multispectral_loss(x_target, x_out)
