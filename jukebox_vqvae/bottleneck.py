...
class BottleneckBlock(nn.Module):
    def __init__(self, k_bins, emb_width, mu):
        super().__init__()
        self.register_buffer('init', t.IntTensor([1]).cuda())
        self.k_bins = k_bins
        self.emb_width = emb_width
        self.mu = mu
        self.threshold = 0.1  #k_bins
        self.register_buffer('init', t.IntTensor([0]).cuda())
        self.register_buffer('k', t.zeros(self.k_bins, self.emb_width).cuda())
        self.register_buffer('k_sum', t.zeros(self.k_bins, self.emb_width).cuda())
        self.register_buffer('k_elem', t.ones(k_bins, device=self.k.device).cuda())
    ...
    def update_k(self, x, x_l):
        ...
        with t.no_grad():
            ...
            # Update centres (no EMA)
            # old_k = self.k
            # self.k_sum[:] = _k_sum  # w, k_bins
            # self.k_elem[:] = _k_elem  # k_bins
            # usage = (self.k_elem.view(k_bins, 1) >= self.threshold).float()
            # self.k[:] = usage * (self.k_sum.view(k_bins, emb_width) / self.k_elem.view(k_bins, 1)) \
            #             + (1 - usage) * _k_rand
            # _k_prob = _k_elem / t.sum(_k_elem)  # x_l_onehot.mean(dim=-1)  # prob of each bin
            ...
            # # Update centres (no random restart)
            # old_k = self.k
            # self.k_sum[:] = mu * self.k_sum + (1. - mu) * _k_sum  # w, k_bins
            # self.k_elem[:] = mu * self.k_elem + (1. - mu) * _k_elem  # k_bins
            # usage = (self.k_elem.view(k_bins, 1) >= self.threshold).float()
            # self.k[:] = self.k_sum.view(k_bins, emb_width) / self.k_elem.view(k_bins, 1)
            # _k_prob = _k_elem / t.sum(_k_elem)  # x_l_onehot.mean(dim=-1)  # prob of each bin
            # entropy = -t.sum(_k_prob * t.log(_k_prob + 1e-8))  # entropy ie how diverse
            # used_curr = (_k_elem >= self.threshold).sum()
            # usage = t.sum(usage)
            ...
            # # Update centres (no EMA, no random restart)
            # old_k = self.k
            # usage = (_k_elem.view(k_bins, 1) >= self.threshold).float()
            # self.k[:] = _k_sum.view(k_bins, emb_width) / _k_elem.view(k_bins, 1)
            # _k_prob = _k_elem / t.sum(_k_elem)  # x_l_onehot.mean(dim=-1)  # prob of each bin
            # entropy = -t.sum(_k_prob * t.log(_k_prob + 1e-8))  # entropy ie how diverse
            # used_curr = (_k_elem >= self.threshold).sum()
            # usage = t.sum(usage)
    ...
    def forward(self, x, update_k=True):
        ...
        if update_k and not self.init[0]:
            self.init_k(x)
